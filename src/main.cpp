...
// Copyright (c) 2013-2014 The Dragoncoin developers
...
uint256 hashGenesisBlock("0x386f489d67b4526a261d1339162e1664c89b4614d64eb6dd117dfadcdb72deca");
...
const string strMessageMagic = "Dragoncoin Signed Message:\n";
...
{
    // Dragoncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dragoncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dragoncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Dragoncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4f12a088393e4ced88f08f47a4444af3c6935bd1bfaa47d5adab47f5f919b593");
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DragoncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DragoncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DragoncoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running DragoncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DragoncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DragoncoinMiner, pwallet));
}
